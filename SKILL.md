...
- **科学偏移票**：最低信念矩阵行替换为固定第 5 注，保留 4 个核心球，并从独立反证候选中通过结构约束搜索选择 2 个偏移球
...
- `team` 模式固定输出 `5` 注 `6+1`
- 核心管线：`_load_patches()` → `differential_learning()` → 8 专家提案 → 核心池/全33红球评分 → 蓝球引擎 → 矩阵出票 + 第5注科学偏移 → 归档
...
- `--team-backtest` 会输出进度和最终 5 注口径指标（含组合平均重叠度）；默认使用 clean 配置且不加载当前归档补丁
