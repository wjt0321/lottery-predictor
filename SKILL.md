...
- `--team-stability-backtest` 会配对运行 dynamic/legacy 多窗口多 seed，输出分位数、bootstrap 置信区间、分组目标差和稳健分，可导出 JSON/CSV，且不写归档
- `--team-threshold-calibration` 使用扩展时间窗在旧训练前缀选阈值、在紧随其后的未见区间验证；只导出报告，不自动改 param patch
- stability/calibration 会复用有界样本上下文缓存并输出遥测；比较结果时除去遥测字段后应与未缓存路径一致
- `parameter_promotion.py` 对折数、验证样本、阈值集中度、正收益比例/均值/CI 和 stability 证据设门禁；只写候选补丁，拒绝覆盖 `param_patch.latest.json`
- 归档分析会按 schema/commit/runtime/patch 版本分组，缺失溯源字段归入 `legacy-unversioned`，并导出 `.versions.csv`
...
- 稳定性回测：`python predict.py --team-stability-backtest --stability-windows 36,72,108,144 --stability-seeds 7,42,101,202,777,2026 --stability-export-prefix prediction_archive/stability_report`
- 阈值滚动校准：`python predict.py --team-threshold-calibration --calibration-train-cycles 36 --calibration-validation-cycles 12 --calibration-folds 3 --calibration-seeds 42 --calibration-export-prefix prediction_archive/threshold_calibration`
- 参数晋升审核：`python parameter_promotion.py --calibration-report prediction_archive/threshold_calibration.json --stability-report prediction_archive/stability_report.json --output prediction_archive/promotion_decision.json --candidate-patch-output config/param_patch.candidate.json`
...
- 动态偏移稳定性：运行 `--team-stability-backtest --stability-windows 36,72,108,144 --stability-seeds 7,42,101,202,777,2026`，需要机器可读结果时加 `--stability-export-prefix`
- 阈值校准：运行 `--team-threshold-calibration`，只接受 expanding-window 验证结果，不在同一时间段选参并报最终成绩
- 参数晋升：校准后运行 `parameter_promotion.py`；`hold` 时不写候选，`eligible` 时也只写 candidate，必须人工审阅后另行激活
...
- 归档文件保留 `ticketN_explain_json` 与 `lead_summary`，并追加 `archive_schema_version`、runtime/patch hash、seed、git commit 可复现元数据
- `team-cover` 预测会写入 `prediction_archive`；`team-cover-backtest`、`team-stability-backtest` 与 `team-threshold-calibration` 都只读历史数据且不写归档
