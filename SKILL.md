...
- **位置权重矩阵**：矩阵每行独立按历史位置表现调整排序，排序后严格切片取前 6
- **增强蓝球引擎**：归一化区间 [0.1, 3.0]，区分度扩大 2.9 倍；冷号配额提升至 2 个，bonus 1.25
- **多样性约束**：旋转矩阵注间红球重叠 ≥4 时自动交换低共识号码（最优替代 + 最多 3 次迭代）
- **权重修复**：`weight_deltas` 强制总和为 0，避免全为负数
- **回测降噪**：差异学习增加 10 次采样取平均，降低单次随机噪声
- **矩阵动态淘汰**：自动排除表现低于平均分 80% 的矩阵行
